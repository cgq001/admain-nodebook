...
export default {
    ...
    methods:{
        handleClickC:function(row){
            this.$router.push('/tablebox/tableTextlist/'+row._id)
        },
                                ...
                                this.$confirm('此标签下还有文章，无法删除', '提示', {
                                  ...
                                  this.$http({
                                        ...
                                        if(res.data.code==0){
                                                ...
                                                this.$store.commit('deleteTable',row._id)
                                                this.numbersArr()
                                        }else{
                    ...
                    this.$http({
                       ...
                       // console.log(res)
                        if(res.data.code==0){
                            ...
                            this.$store.commit('undateTable',this.form)
                        }else{
                            this.dialogFormVisible = false
                            this.$message.error('未知错误');
                        }
        ...
        pageStr:function(num=1){ // 查询全部标签
            this.$http({
                method:'POST',
                url: 'searchTablesLists',
                data:{
                    NumStr:this.NumStr,
                    num:num
                }
