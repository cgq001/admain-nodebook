...
export default {
    data() {
      return {
        breadcrumb:['标记管理','标签管理','全部标签','标签详情'],
        tableData: [],
        Numbers:1,  //总条数
        NumStr:10 ,  //每页显示条数
        // tablesList:[] , //全部标签
     ...
     methods: {
      ...
      handleClick(row) {   //点击查看
            if(row.bst==2){
                this.$router.push({path:'/tablebox/undatemark/'+row.id})
            }else{
                this.$router.push({path:'/tablebox/edit',query:{info:row.id}})
            }
