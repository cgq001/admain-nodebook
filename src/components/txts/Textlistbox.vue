...
export default {
    data() {
     ...
     methods: {
      handleClick(row) {   //点击查看undatemark
        if(this.$store.state.user.desc>1){
            ...
            if(row.bst==2){
                this.$router.push({path:'/textlist/undatemark/'+row.id})
            }else{
                this.$router.push({path:'/textlist/edit',query:{info:row.id}})
            }
      ...
      previewClick:function(val){
         
          window.location.href='http://text.nodebook.top/text/'+val.id
      },
